...
export class AuthController {
    ...
    @Post('/signup/user')
    @ApiOperation({ summary: '개인 회원가입 1단계', description: '기존 회원인지 확인하기 위한 사용자 정보' })
    @ApiCreatedResponse({ description: '찾거나 생성한 사용자 정보 반환', type: SignupUserResponse })
    ...
    @Post('/signup/account')
    @ApiOperation({ summary: '개인 회원가입 2단계', description: '개인 회원가입' })
    @ApiCreatedResponse({ description: '계정을 생성한다.', type: SignupAccountResponse })
    ...
    @ApiCreatedResponse({ description: '기업을 등록하고 본인의 계정을 기업에 연결한다.', type: SignupCompanyResponse })
    @UseGuards(AuthGuard)
    ...
    @Get('/account/me')
    @ApiOperation({ summary: '내 계정 정보', description: '내 계정 정보 가져오기' })
